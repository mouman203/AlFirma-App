...
class HireWorkerPage extends StatefulWidget {
  ...
  @override
  State<HireWorkerPage> createState() => _HireWorkerPageState();
}

class _HireWorkerPageState extends State<HireWorkerPage> {
  String? selectedWilaya;
  String? selectedDaira;
  static Future<List<Products>> getJobRequestOnce() async {
    final snapshot = await FirebaseFirestore.instance
        .collection('item')
        .doc('Services')
        .collection('Services')
        .where('typeItem', isEqualTo: "طلب عمل")
        .get();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(S.of(context).hireWorker,style:TextStyle(fontWeight: FontWeight.bold)),
        elevation: 5,
      ),
            ...
            FutureBuilder<List<Products>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return Center(child: Text(S.of(context).no_workers_found));
                }
